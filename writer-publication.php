...
<head>
    ...
    <link rel="stylesheet" href="css/borrow-books.css">
    <style>
        .profile-pic{
            width: 50px;
            height: 50px;
...
<?php
  ...
  $publishers = $result->fetch_all(MYSQLI_ASSOC); // Fetch all records as associative array

  $writer_id = $_SESSION['user_id'];
  $query = "SELECT * from publication_request join publishers on publication_request.publisher_id = publishers.publisher_id
   WHERE writer_id = $writer_id order by request_date desc ";
  $stmt = $db->prepare($query);
  $stmt->execute(); // Execute the statement
  $result = $stmt->get_result(); // Get the result
  $requests = $result->fetch_all(MYSQLI_ASSOC); // Fetch all records as associative array
?>
...
<div id="myModal" class="modal">
    <div class="modal-content">
        ...
        <form id="add-request-form" method="POST">
            <label for="">Choose Publisher</label>
            <select id="status" name="publisher_id" required>
                <?php if($publishers){ foreach($publishers as $publisher){ ?>
            ...
            <textarea id="manuscriptLink" name="manuscript" placeholder="Manuscript Link (e.g., URL)" required></textarea>
            <button name="publication_request" type="submit">Submit</button>
        </form>
    </div>
</div>
<?php  
         if(isset($_POST['publication_request'])){
            ...
            // Prepare the SQL insert query
                $sql = "INSERT INTO publication_request (publisher_id, writer_id, manuscript, genre, request_date, status, book_name)
                VALUES (?, ?, ?, ?, NOW(), 'pending', ?)";

            // Prepare the statement
            if ($stmt = $db->prepare($sql)) {
            // Bind the parameters
            $stmt->bind_param('iisss', $publisher_id, $writer_id, $manuscript, $genre, $book_name);

            // Execute the statement
            if ($stmt->execute()) {
                // Use Post/Redirect/Get (PRG) pattern to prevent form re-submission on reload
                header("Location: writer-publication.php"); // Redirect to a success page (or the same page)
                exit(); // Ensure the script stops executing after the redirect
            } else {
                echo "Error: Could not insert the request.";
            }
...
?>
    <div class="site-wrapper" id="top">
            ...
            <section class="account-context">
                <div class="inner-account-context">
                    <div class="tablist">
                        ...
                        <div class="tabs">
                            <div class="tab-divs ">
                                <a href="writer-account.php" ><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
                            ...
                            <div class="tab-divs ">
                                <a href="#"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
                                  ...
                                  </svg><span class="tab-title">REVIEWS</span> </a>
                            </div>
                            <div class="tab-divs ">
                                <a href="#"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
                                    <path d="M2.66663 13.3333V2.66667H6.66663V3.33333H3.33329V12.6667H12.6666V8.664L13.3333 9.33067V13.3333H2.66663ZM4.99996 11L6.30796 9.282L7.47463 10.6927L9.21796 8.48733L11.1026 11H4.99996ZM14.1866 8.318L12.0946 6.226C11.8702 6.39 11.6524 6.50444 11.4413 6.56933C11.2302 6.63422 10.9917 6.66667 10.726 6.66667C10.0744 6.66667 9.52396 6.44133 9.07463 5.99067C8.62485 5.53911 8.39996 4.98667 8.39996 4.33333C8.39996 3.68 8.62551 3.12778 9.07663 2.67667C9.52774 2.22556 10.08 2 10.7333 2C11.3866 2 11.9391 2.22556 12.3906 2.67667C12.8422 3.12778 13.0675 3.68 13.0666 4.33333C13.0666 4.59911 13.0266 4.85178 12.9466 5.09133C12.8666 5.33089 12.7511 5.56267 12.6 5.78667L14.6593 7.84533L14.1866 8.318ZM10.7333 6C11.2 6 11.5944 5.83889 11.9166 5.51667C12.2388 5.19444 12.4 4.8 12.4 4.33333C12.4 3.86667 12.2388 3.47222 11.9166 3.15C11.5944 2.82778 11.2 2.66667 10.7333 2.66667C10.2666 2.66667 9.87218 2.82778 9.54996 3.15C9.22774 3.47222 9.06663 3.86667 9.06663 4.33333C9.06663 4.8 9.22774 5.19444 9.54996 5.51667C9.87218 5.83889 10.2666 6 10.7333 6Z" fill="black"/>
                                  </svg><span class="tab-title">NOTIFICATION</span> </a>
                            </div>
        ...
        <button class="button" id="openModalBtn" >Add New Request</button>
    </div>
...
<div class="filter-section">
    ...
    <input type="text" id="book-name-filter" placeholder="Search by Book Name" onkeyup="filterTable()">
    <input type="text" id="publisher-name-filter" placeholder="Search by Publisher Name" onkeyup="filterTable()">
    <input type="text" id="genre-filter" placeholder="Search by Genre" onkeyup="filterTable()">
</div>
...
<table id="manuscript-table">
    ...
    <tbody>
        ...
        <tr>
            <td><?php echo $request['publisher_name'] ?></td>
            <td><?php echo $request['book_name'] ?></td>
            <td><?php echo $request['phone_number'] ?></td>
            <td><?php echo $request['genre'] ?></td>
            <td><a href="<?php echo $request['manuscript'] ?>" class="view-icon" target="_blank">📄</a></td>
            <td>
            <?php if($request['status']=="pending"){ ?>
                <span class="status Pending"><?php echo $request['status'] ?></span>
                <?php } ?>
                <?php if($request['status']=="accepted"){ ?>
                <span class="status Accepted"><?php echo $request['status'] ?></span>
                <?php } ?>
                <?php if($request['status']=="declined"){ ?>
                <span class="status Declined"><?php echo $request['status'] ?></span>
                <?php } ?>
            </td>
    ...
    function filterTable() {
        // Get filter values
        let statusFilter = document.getElementById("status-filter").value.toUpperCase();
        let bookNameFilter = document.getElementById("book-name-filter").value.toUpperCase();
        let publisherNameFilter = document.getElementById("publisher-name-filter").value.toUpperCase();
        let genreFilter = document.getElementById("genre-filter").value.toUpperCase();

        // Get the table and rows
        let table = document.getElementById("manuscript-table");
        let rows = table.getElementsByTagName("tr");

        // Loop through the rows and hide/show based on filters
        for (let i = 1; i < rows.length; i++) {
            let cells = rows[i].getElementsByTagName("td");
            let publisherName = cells[0].textContent.toUpperCase();
            let bookName = cells[1].textContent.toUpperCase();
            let genre = cells[3].textContent.toUpperCase();
            let status = cells[5].textContent.toUpperCase();

            // Apply filters
            if (
                (statusFilter === "ALL" || status === statusFilter) &&
                (publisherName.includes(publisherNameFilter)) &&
                (bookName.includes(bookNameFilter)) &&
    ...
    // Placeholder filterTable function (you can replace it with your actual filter function)
    function filterTable() {
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="bookModalLabel">Add a New Book</h5>
      ...
      <div class="modal-body">
        <form id="bookForm">
          ...
          <div class="mb-3">
            <label for="bookUrl" class="form-label">URL</label>
            <input type="url" class="form-control" id="bookUrl" placeholder="Enter book URL">
          </div>
          <div class="mb-3">
      ...
      <div class="modal-footer">
        ...
        <button type="submit" class="btn btn-primary" form="bookForm">Save Book</button>
      </div>
    </div>
  </div>
</div>
