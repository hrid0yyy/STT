...
<!-- image search -->
         ...
         <section id="main-content">
            ...
            <section class="account-context">
                <div class="inner-account-context">
                    ...
                    <div class="profile">
                        ...
                        <div class="tabs">
                            <div class="tab-divs active">
                                <a href="account-reader.php" style="color: white"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
                    ...
                    <section class="bookshelf-div" style=" width:100% ;" >
                        

                            <div class="bookshelf-container">
                                <h4 style="text-align: center;">My Bookshelf</h4>
                                <div id="shelfContainer">
                                    <!-- Default rows will be added here -->
                                </div>
                                <div style="display: flex; justify-content: flex-end;">
                                  <button id="addRowBtn" style="padding:1rem; border-radius:0.25rem;background-color: #ffff;">Add New Row</button>
                                </div>
    ...
    fetchBooks(); // Fetch saved books when the page loads
});
...
// Function to fetch books from the database and display them in rows
function fetchBooks() {
    fetch('backend/get-books.php')
        .then(response => response.json())
        .then(books => {
            books.forEach(book => {
                ensureRowExists(book.shelf_row); // Make sure the row exists
                addBookToRowFromDatabase(book.shelf_row, book.book_title, book.book_pdf_url); // Add books to row
            });
        })
        .catch(error => {
...
// Ensure a specific row exists, if not create it
function ensureRowExists(rowIndex) {
    const shelfContainer = document.getElementById('shelfContainer');
    if (!document.getElementById(`bookList${rowIndex}`)) {
        addNewRow();
    }
}

// Function to add a book to a row (from database)
function addBookToRowFromDatabase(rowIndex, title, pdfUrl) {
    const bookList = document.getElementById(`bookList${rowIndex}`);
    const bookDiv = document.createElement('div');
    bookDiv.classList.add('book');
    bookDiv.innerHTML = `<img src="https://via.placeholder.com/100x150.png?text=PDF" alt="${title}" />`;
...
// Function to open the PDF in the viewer
...
// Function to add a new row to the shelf
function addNewRow() {
    const shelfContainer = document.getElementById('shelfContainer');
...
function deleteRow(btn) {
    ...
    const shelfContainer = document.getElementById('shelfContainer');
    row.remove();
    updateRowSelector(); // Update row selector options
}

function updateRowSelector() {
    const rowSelector = document.getElementById('rowSelector');
    rowSelector.innerHTML = ''; // Clear options
...
// Function to add a book to a row (from user file input)
// Replace the 'userId: 1' with dynamic user session id in JavaScript
    ...
    if (file instanceof File) {
        ...
        fetch('backend/save-book.php', {
            ...
            body: JSON.stringify({
                ...
                shelfRow: rowIndex,
                userId: <?php echo $_SESSION['user_id']; ?> // Use PHP to inject session user ID dynamically
            }),
        })
        .then(response => response.json())
        .then(result => {
