...
<?php include 'backend/shopowner/inventory.php'; ?>
<head>
    <meta charset="utf-8" />
                        ...
                        <div class="tabs">
                            ...
                            <div class="tab-divs active">
                                <a href="#"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
                                    ...
                                    </svg><span class="tab-title"><a href="#" style="color: white;">Inventory</a></span>
                        ...
                        <div class="heading-container">
                            <div class="heading-area">
                                <p style="line-height: 1.35rem;">
                                    <span style="font-size: 18px; font-weight: bolder;">Product List</span> <br>Manage your products
                                </p>
                            </div>
...
<br>
                        <div class="product-filters">
                                ...
                                <select id="genre-filter">
                                    <?php include 'backend/basicOperation.php';
                                    $lists = genre();
                                    ?>
                                    <option value="">All</option>
                                    <?php foreach ($lists as $list) { ?>
                                    <option value="<?php echo $list['genre'] ?>"><?php echo $list['genre'] ?></option>
                                     <?php  } ?>
                                    <!-- Add other genres -->
                                </select>
                           ...
                           <input type="text" id="search-bar" placeholder="Search by book title">
                            </div>
                        ...
                        <div class="product-table">
                                <table id="product-table">
                                    ...
                                    <?php $items = getItems($_SESSION['user_id']); ?>
                                    <?php if ($items) { 
                                        foreach ($items as $item) { ?>
                                            <tr data-genre="<?php echo $item['genre']; ?>" data-quality="<?php echo $item['quality']; ?>">
...
<script>
   document.addEventListener('DOMContentLoaded', function() {
    ...
    function filterTable() {
        ...
        for (let i = 1; i < rows.length; i++) { // Start from 1 to skip header
            const genre = rows[i].getAttribute('data-genre').toLowerCase();
            const quality = rows[i].getAttribute('data-quality').toLowerCase();
            const title = rows[i].getElementsByTagName('td')[0].textContent.toLowerCase(); // Title is in the first td
            ...
            // Show or hide the row based on filters and search query
            if (matchesGenre && matchesQuality && matchesTitle) {
    ...
    // Add event listeners for filter change and search bar input
    genreFilter.addEventListener('change', filterTable);
    qualityFilter.addEventListener('change', filterTable);
    searchBar.addEventListener('keyup', filterTable); // Trigger on keyup for real-time search
});
  ...
  function handleFileUpload(event) {
    ...
    if (file) {
      ...
      // You can now handle the file as needed (e.g., upload to server)
    }
  }
    ...
    document.getElementById('addProductModal').style.display = displayStyle;
}
...
function saveProduct() {
    ...
    .then(data => {
        ...
        cell1.innerHTML = `<img src="" alt=""> ${formData.get('book_name')}`;
        cell2.innerHTML = formData.get('genre');
        cell3.innerHTML = `$${parseFloat(formData.get('price')).toFixed(2)}`; // Display the price
        cell4.innerHTML = 'Available';
        cell5.innerHTML = '<button onclick="deleteRow(this)">Delete</button>';

        closeModal(); // Close modal after saving product
    });
}
