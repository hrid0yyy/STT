                        ...
                        <div class="heading-container">
                            <div class="heading-area">
                                <p style="line-height: 1.35rem;">
                                    <span style="font-size: 18px; font-weight: bolder;">Purchase List</span> <br>track your purchasing
                                </p>
                            </div>
...
<div class="filter-container">
        <input type="text" id="searchBookName" placeholder="Search by Book Name" onkeyup="filterTable()">
        <input type="text" id="searchShopName" placeholder="Search by Shop Name" onkeyup="filterTable()">
        <input type="date" id="startDate" placeholder="Start Date" onchange="filterTable()">
        <input type="date" id="endDate" placeholder="End Date" onchange="filterTable()">
...
$user_id = $_SESSION['user_id'];
            ...
            $sql = "select bookcover,title,quantity,shop_name,status,purchase_date from purchase 
            join readers on purchase.reader_id = readers.reader_auto_id
            join shopowners on purchase.shop_owner_id = shopowners.shop_owner_id
            join billing on purchase.purchase_id = billing.purchase_id
            join books on books.book_id = purchase.book_id
            where readers.reader_auto_id = ?
...
?>
    ...
    <table id="purchaseHistoryTable">
        ...
        <tbody>
            ...
            <tr>
                <td><img src="image/book-cover/<?php echo $item['bookcover'] ?>"  alt="Book cover image" 
                            onerror="this.onerror=null;this.src='image/book-cover/default.png';"></td>
                <td class="bookName"><?php echo $item['title'] ?></td>
                <td><?php echo $item['quantity'] ?></td>
                <td class="shopName"><?php echo $item['shop_name'] ?></td>
                <td class="purchaseDate"><?php echo $item['purchase_date'] ?></td>
...
function filterTable() {
    ...
    const searchShopName = document.getElementById("searchShopName").value.toLowerCase();
    ...
    for (let i = 1; i < rows.length; i++) {
        const bookName = rows[i].getElementsByClassName("bookName")[0].textContent.toLowerCase();
        const shopName = rows[i].getElementsByClassName("shopName")[0].textContent.toLowerCase();
        const purchaseDate = rows[i].getElementsByClassName("purchaseDate")[0].textContent;
        const status = rows[i].getElementsByTagName("span")[0].textContent;

        let matchesBookName = bookName.includes(searchBookName);
        let matchesShopName = shopName.includes(searchShopName);
        ...
        if (startDate && endDate) {
            ...
            matchesDate = purchaseDate >= startDate;
        } else if (endDate) {
            matchesDate = purchaseDate <= endDate;
        }
        ...
        if (matchesBookName && matchesShopName && matchesDate && matchesStatus) {
