...
<head>
    ...
    <title>E-book | Shelf To Tales</title>
    ...
    <link rel="stylesheet" href="css/e-book-reader.css">
    <style>
        .profile-pic{
            width: 50px;
            height: 50px;
...
// Query to fetch the books uploaded by the user along with the reading progress
$sql = "SELECT id, book_title, writer, image_path, progress, pdf_path FROM bookshelf_pdfs WHERE user_id = ?";
$stmt = $conn->prepare($sql);
$stmt->bind_param("i", $user_id);
$stmt->execute();
$result = $stmt->get_result();

$total_books = $result->num_rows; // Get total number of books
?>

<div class="header">
    <span>My Books (<?php echo $total_books; ?>)</span> <!-- Dynamic book count -->
</div>

<div class="ebooks-list" style="display: flex; flex-wrap: wrap; gap: 20px;">
    <?php
    if ($total_books > 0) {
        while ($row = $result->fetch_assoc()) {
            echo '<div class="book-card" style="width: calc(25% - 20px); border: 1px solid #ccc; border-radius: 10px; padding: 10px; text-align: center; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1);">';
            echo '<div class="book-cover" style="height: 300px; overflow: hidden;">';
            echo '<img src="' . $row['image_path'] . '" alt="' . htmlspecialchars($row['book_title']) . '" style="width: 100%; height: 100%; object-fit: cover;">';
            echo '</div>';
            echo '<div class="book-name" style="margin: 10px 0; font-weight: bold; font-size: 18px;">' . htmlspecialchars($row['book_title']) . '</div>';
            echo '<div class="extra-info" style="color: #666;">' . htmlspecialchars($row['writer']) . '</div>';

            // Reading progress bar
            $progress = $row['progress'];
            echo '<div class="progress-bar" style="margin-top: 10px;">';
            // echo '<div style="width: ' . $progress . '%; background-color: #62ab00; height: 10px; border-radius: 5px;"></div>';
            echo '</div>';
            // echo '<p style="font-size: 12px;">Progress: ' . $progress . '%</p>';

            // Read book button (Triggers modal)
            echo '<button type="button" class="btn btn-success" onclick="openBookModal(' . $row['id'] . ', \'' . addslashes($row['book_title']) . '\', \'' . addslashes($row['pdf_path']) . '\', ' . $progress . ')">Read Book</button>';
            echo '</div>';
        }
    } else {
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="readBookModalLabel" >Read Book</h5>
            ...
            <div class="modal-body">
                ...
                <form id="updateProgressForm" method="POST" action="update_progress.php">
                    <input type="hidden" name="book_id" id="bookId">
                    <div class="form-group mt-3">
                        <label for="progress"></label>
                        <input type="hidden" class="form-control" name="progress" id="progressInput" min="0" max="100">
                    </div>
...
function openBookModal(bookId, bookTitle, pdfPath, progress) {
    // Set book details in the modal
    ...
    // Show the modal
    var modal = new bootstrap.Modal(document.getElementById('readBookModal'));
    modal.show();
}

// Function to load the PDF and initialize the PDF.js viewer
...
function renderPage(pageNum) {
    pdfDoc.getPage(pageNum).then(function(page) {
        ...
        const pdfViewer = document.getElementById('pdfViewer');
        pdfViewer.innerHTML = ''; // Clear the previous page
        pdfViewer.appendChild(canvas);
    });
}

// Update reading progress based on the current page
function updateProgress() {
    const progress = Math.floor((currentPage / totalPages) * 100);
    document.getElementById('progressInput').value = progress;

    // Auto-save progress after each page view
...
function saveProgress(progress) {
    ...
    // Send progress data via AJAX to update the database
