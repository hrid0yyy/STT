...
$query = "SELECT ba.*, b.title 
          FROM book_advertisements ba
          JOIN books b ON ba.book_id = b.book_id
          WHERE ba.ad_id = ?";
$stmt = $conn->prepare($query);
$stmt->bind_param("i", $ad_id);  // Bind the correct column name for the advertisement ID
$stmt->execute();
$result = $stmt->get_result();

if ($result->num_rows == 0) {
    echo "Error: Advertisement not found!";
    exit();
}

$ad_row = $result->fetch_assoc();

// Handle form submission for updating the advertisement
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $update_query = "UPDATE book_advertisements 
                     SET discount_percentage = ?, advertising_start_date = ?, advertising_end_date = ?
                     WHERE ad_id = ?";  // Use the correct column name for the advertisement ID
    $stmt = $conn->prepare($update_query);
    $stmt->bind_param("issi", $discount, $start_date, $end_date, $ad_id);

    if ($stmt->execute()) {
        echo "<p>Advertisement updated successfully!</p>";
...
<body>
    ...
    <h2>Edit Advertisement for <?php echo htmlspecialchars($ad_row['title']); ?></h2>
    
    <form action="edit_advertisement.php?id=<?php echo htmlspecialchars($ad_id); ?>" method="POST">
        <div>
            <div>
            <label style="font-size:1.5rem" for="discount">Discount Percentage:</label>
            </div>
            <div>
            <input style="width:100%;height:2rem;font-size:1rem; padding:0.25rem" type="number" name="discount" min="0" max="100" value="<?php echo htmlspecialchars($ad_row['discount_percentage']); ?>" required>
            </div>
        </div>
        
        <div>
            <div>
            <label style="font-size:1.5rem" for="start_date">Start Date:</label>
            </div>
            <div>
            <input style="width:100%;height:2rem;font-size:1rem; padding:0.25rem" type="datetime-local" name="start_date" value="<?php echo htmlspecialchars($ad_row['advertising_start_date']); ?>" required>
        ...
        <div>
            <div>
            <label style="font-size:1.5rem" for="end_date">End Date:</label>
            </div>
            <div>
            <input style="width:100%;height:2rem;font-size:1rem; padding:0.25rem" type="datetime-local" name="end_date" value="<?php echo htmlspecialchars($ad_row['advertising_end_date']); ?>" required>
        ...
        <button type="submit" style="padding:1rem;background-color:#62ab00;">Update Advertisement</button>
        </div>
        
    </form>

    <a href="manage_advertisements.php">Back to Manage Advertisements</a>
    </div>
                    </section>
