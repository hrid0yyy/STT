...
$user_id = $_SESSION['user_id'];  // Assuming the `user_id` is stored in the session after login

// Step 1: Fetch `shop_owner_id` from `shopowners` table using the `user_id`
$query = "SELECT shop_owner_id FROM shopowners WHERE shop_owner_id = ?";
...
if ($result->num_rows > 0) {
    ...
    $shop_owner_id = $row['shop_owner_id'];  // Store the `shop_owner_id`
} else {
    echo "Error: Shop owner not found!";
    exit();
}

// Step 2: Fetch available books for the `shop_owner_id` from `book_shopowners`
$query = "SELECT bs.book_id, b.title 
          FROM book_shopowners bs
          JOIN books b ON bs.book_id = b.book_id
          WHERE bs.shop_owner_id = ? AND bs.stock_quantity > 0";  // Only fetch books that are in stock
$stmt = $conn->prepare($query);
$stmt->bind_param("i", $shop_owner_id);  // Bind the `shop_owner_id` to the query
$stmt->execute();
$books_result = $stmt->get_result();

// Step 3: Handle form submission for advertisement creation
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Insert the advertisement data into the database
    $query = "INSERT INTO book_advertisements (book_id, shop_owner_id, discount_percentage, advertising_start_date, advertising_end_date, status) 
              VALUES (?, ?, ?, ?, ?, 'active')";
    $stmt = $conn->prepare($query);
    $stmt->bind_param("iiiss", $book_id, $shop_owner_id, $discount, $start_date, $end_date);

    if ($stmt->execute()) {
        echo "<p>Advertisement created successfully!</p>";
    } else {
...
<body>
    ...
    <form action="create_advertisement.php" method="POST" style="">
        <div style="display:flex;justify-content:center;align-item:center;flex-direction:column">
            <div>
                <label for="book" style="font-size:1.5rem">Select Book:</label>
            </div>
        <select name="book_id" required style="height:2rem;font-size:1rem; padding:0.25rem">
            <?php
            // Step 4: Populate the select dropdown with available books from the shop owner's inventory
            if ($books_result->num_rows > 0) {
                while ($row = $books_result->fetch_assoc()) {
                    echo "<option value='" . $row['book_id'] . "'>" . $row['title'] . "</option>";
                }
            } else {
                echo "<option disabled>No books available in your inventory</option>";
            }
            ?>
        </select>
        </div>
            ...
            <div>
                <div>
                    <label for="discount" style="font-size:1.5rem">Discount Percentage:</label>
                </div>
                <div >
                    <input style="width:100%;height:2rem;font-size:1rem; padding:0.25rem"type="number" name="discount" min="0" max="100" placeholder="30%" required>
                </div>
            </div>
     
        <div>
                <div>
                <label for="start_date" style="font-size:1.5rem">Start Date:</label>
                </div>
                <div>   
                <input style="width:100%;height:2rem;font-size:1rem; padding:0.25rem" type="datetime-local" name="start_date" required>
                </div>
            </div>
        ...
        <div>
                <div>
                    <label for="end_date" style="font-size:1.5rem">End Date:</label>
                </div>
                <div>
                    <input style="width:100%;height:2rem;font-size:1rem; padding:0.25rem" type="datetime-local" name="end_date" required>
                </div>
            </div>
        ...
        <div>
            ...
            <div style="display:flex; justify-content:flex-end;margin-top:1rem">
                <button type="submit" style="padding:1rem; background-color:#62ab00;">Create Advertisement</button>
            </div>
