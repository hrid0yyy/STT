...
$ad_id = $_GET['id'];  // Assuming 'id' is passed via URL

// Step 1: Fetch the advertisement with the book title from the books table
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $delete_query = "DELETE FROM book_advertisements WHERE ad_id = ?";  // Use the correct column name for the advertisement ID
    $stmt = $conn->prepare($delete_query);
    $stmt->bind_param("i", $ad_id);

    if ($stmt->execute()) {
        echo "<p>Advertisement deleted successfully!</p>";
...
<body>
    ...
    <h2>Delete Advertisement for "<?php echo htmlspecialchars($ad_row['title']); ?>"</h2>
    <p>Are you sure you want to delete this advertisement?</p>

    <form action="delete_advertisement.php?id=<?php echo htmlspecialchars($ad_id); ?>" method="POST">
        <button type="submit">Yes, Delete</button>
    </form>

    <a href="manage_advertisements.php">Cancel</a>
    </div>
                    </section>
