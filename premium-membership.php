...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $type = 'premium'; // Type is now premium for this page
    ...
    if ($result->num_rows > 0) {
        ...
        if ($row['type'] === 'elite') {
            ...
            // Update membership to premium if it's currently free
            $update_sql = "UPDATE membership SET type = ?, date = ? WHERE reader_id = ?";
            $update_stmt = $conn->prepare($update_sql);
            $update_stmt->bind_param("ssi", $type, $date, $reader_id);
            if ($update_stmt->execute()) {
                echo "<script>alert('Membership upgraded to Premium!'); window.location.href = 'borrow-list.php';</script>";
            } else {
        ...
        // No existing membership, insert new premium membership
        $sql = "INSERT INTO membership (reader_id, type, date) VALUES (?, ?, ?)";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("iss", $reader_id, $type, $date);

        if ($stmt->execute()) {
            echo "<script>alert('Premium Membership signup successful!'); window.location.href = 'borrow-list.php';</script>";
        } else {
            echo "<script>alert('Error: " . $conn->error . "');</script>";
        }
...
<body>
    <div class="membership-container">
        ...
        <ul>
            ...
            <li>You can borrow books up to 1000 Taka.</li>
            <li>Books can be borrowed for a maximum of 30 days.</li>
        ...
        <p class="bkash-info">To complete your Premium Membership purchase, please follow the instructions below to make a payment through bKash:</p>
        
        <div class="payment-instructions">
            <p><strong>bKash Number:</strong> 01XXXXXXXXX</p>
            <p><strong>Reference:</strong> Premium Membership</p>
            <p><strong>Amount:</strong> 1000 Taka</p>
        </div>
